...
ModelOutput = namedtuple('ModelOutput',
                         ['vertices','faces', 'joints', 'full_pose', 'betas',
                          ...
                          'jaw_pose', 'T', 'T_weighted', 'weights', 'l2w'])
...
class SMPL(nn.Module):
    ...
    def forward(self, betas=None, body_pose=None, global_orient=None,
        ...
        if displacement is not None:
            vertices, joints_smpl, T_weighted, W, T, l2w = lbs(betas, full_pose, v_template+displacement,
                                   self.shapedirs, self.posedirs,
                                   self.J_regressor, self.parents,
                                   self.lbs_weights, dtype=self.dtype,pose_blend=self.pose_blend)
        else:
            vertices, joints_smpl,T_weighted, W, T, l2w = lbs(betas, full_pose, v_template,
                                        self.shapedirs, self.posedirs,
                                        self.J_regressor, self.parents,
                                        self.lbs_weights, dtype=self.dtype,pose_blend=self.pose_blend)
        ...
        output = ModelOutput(vertices=vertices if return_verts else None,
                             ...
                             T=T, T_weighted=T_weighted, weights=W, l2w=l2w)
        return output