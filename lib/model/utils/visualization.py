...
def plot_points3d_mpl(points, val_range=1.2, marker_size=1.):
    ...
    ax.scatter(points[:,0], points[:,1], points[:,2], c='red', s=marker_size)
...
def plot_boxes_mpl(corners, joints, val_range=1.2, marker_size=1., alpha=0.3, line_color='teal'):
    fig = plt.figure()
    ax = fig.add_subplot(projection='3d')
    for box in corners:
        box = corners[0]
    ...
    ax.scatter(joints[:,0], joints[:,1], joints[:,2], marker='^', c='blue', s=marker_size*10)
...
def draw_opt_poses(c2ws, hwf, anchor_kps, opt_kps, gt_kps=None,
    ...
    for i, (anchor_2d, opt_2d) in enumerate(zip(anchors_2d, opts_2d)):
        img = base_imgs[i]
        img = draw_markers(opt_2d, img=img, H=h, W=w, res=res, thickness=thickness, marker_size=marker_size,
                           color=(255, 149, 0, 32))
